...
Iso.rightInv (puppeTotalIso1 B C f) p =
  cong (sym (snd f) ∙_) (lUnit _ ⁻¹) ∙ assoc (sym (snd f)) (snd f) p
  ∙ cong (_∙ p) (lCancel _) ∙ lUnit _ ⁻¹
Iso.leftInv (puppeTotalIso1 B C f) (b , (p , q)) =
  ΣPathP ((sym q) ,
  (toPathP (≡-×
   (transportFunPathLemma' b (pt B) (pt C) (fst f) q
   (snd f ∙ sym (snd f) ∙ cong (fst f) (sym q) ∙ p)
   ∙ cong (cong (fst f) q ∙_)
          (assoc (snd f) (sym (snd f)) (cong (fst f) (sym q) ∙ p)
          ∙ cong (_∙ (cong (fst f) (sym q) ∙ p)) (rCancel _))
   ∙ cong (cong (fst f) q ∙_) (lUnit _ ⁻¹)
   ∙ assoc (cong (fst f) q) (cong (fst f) (sym q)) p
   ∙ cong (_∙ p) (rCancel _)
   ∙ lUnit _ ⁻¹)
...
Iso.leftInv (puppeFiberIso1 B C f) ((b , p) , (q , r)) =
  ΣPathP ((sym r) ,
   (toPathP (≡-×
   (transportPathLemma' _ _ _ (λ i → fst (r i)) _
    ∙ assoc (λ i → fst (r i)) (λ i → fst (r (~ i))) q
    ∙ cong (_∙ q) (rCancel (λ i → fst (r i)))
    ∙ lUnit q ⁻¹)
...
puppeFiberFiberCase {B = B} {C = C} f =
  FiberIsoFiberSeq (fiber∙ (inclOfFiberFiberSeq f)) (Ω C) (fiber∙ f) B
  (fst (puppeTotalIso f)) (snd (puppeTotalIso f))
  (FiberFiberSeq (fiber∙ f) B (inclOfFiberFiberSeq f))
  
puppe : {X Y Z : Pointed ℓ} → FiberSeq X Y Z → FiberSeq (Ω Z) X Y
puppe F = TotalIsoFiberSeq (Ω (FiberSeqBase F)) (fiber∙ (FiberSeqProj F))
                          (FiberSeqFiber F) (FiberSeqTotal F)
          (fst (FibsIsoFiberFiberSeq F)) (snd (FibsIsoFiberFiberSeq F))
...
puppeProjEqFibIncl F =
  TotalIsoFiberSeqProj
    ...
    (puppeFiberFiberCase (FiberSeqProj F))
  ∙ cong (_∘∙ (Iso.inv (fst (FibsIsoFiberFiberSeq F)) ,
                 cong (Iso.inv (fst (FibsIsoFiberFiberSeq F)))
                      (sym (snd (FibsIsoFiberFiberSeq F)))
                 ∙ Iso.leftInv (fst (FibsIsoFiberFiberSeq F)) _))
         ((FiberIsoFiberSeqProj (fst (puppeTotalIso (FiberSeqProj F)))
                               (snd (puppeTotalIso (FiberSeqProj F)))
                               (FiberFiberSeq _ _
                               (inclOfFiberFiberSeq (FiberSeqProj F))))
          ∙ ProjOfFiberFiberSeq (inclOfFiberFiberSeq (FiberSeqProj F))
          ∙ sym (InclOfFiberFiberSeq (FiberSeqProj F)))
  ∙ snd (FibsIsoOfFibSeq (FiberFiberSeq _ _ (FiberSeqProj F)) F
...
alternativeIteratedPuppeFiberFiberCase f =
  FiberIsoFiberSeq _ _ _ _
    (fst (puppeFiberIso f)) (snd (puppeFiberIso f))
    (TotalIsoFiberSeq _ _ _ _
      (fst (puppeTotalIso f)) (snd (puppeTotalIso f))
      (FiberFiberSeq (fiber∙ (inclOfFiberFiberSeq f)) (fiber∙ f)
      (inclOfFiberFiberSeq (inclOfFiberFiberSeq f))))

alternativeIteratedPuppe : {A B C : Pointed ℓ} → FiberSeq A B C
  → FiberSeq (Ω B) (Ω C) A
alternativeIteratedPuppe F =
  BaseIsoFiberSeq (Ω (FiberSeqTotal F)) (Ω (FiberSeqBase F))
  (fiber∙ (FiberSeqProj F)) (FiberSeqFiber F)
  (fst (FibsIsoFiberFiberSeq F)) (snd (FibsIsoFiberFiberSeq F))
...
altIteratedPuppeEqIteratedPuppe F =
   EqOfEqFiberSeqProj (alternativeIteratedPuppe F) (puppe (puppe F))
 ...
 ≡⟨ (BaseIsoFiberSeqProj
  ...
  ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
     snd (FibsIsoFiberFiberSeq F))
    ∘∙
    (FiberSeqProj (alternativeIteratedPuppeFiberFiberCase (FiberSeqProj F))))
 ≡⟨ cong ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
             snd (FibsIsoFiberFiberSeq F)) ∘∙_)
            (FiberIsoFiberSeqProj _ _ _) ⟩
  ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) , snd (FibsIsoFiberFiberSeq F))
  ∘∙
  FiberSeqProj
  (TotalIsoFiberSeq _ _ _ _ _ _ _))
 ≡⟨ cong ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
            snd (FibsIsoFiberFiberSeq F)) ∘∙_)
         (TotalIsoFiberSeqProj _ _ _) ⟩
  ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) , snd (FibsIsoFiberFiberSeq F))
  ∘∙ (FiberSeqProj (FiberFiberSeq _ _
      (inclOfFiberFiberSeq (inclOfFiberFiberSeq (FiberSeqProj F))))
  ∘∙ (Iso.inv (fst (puppeTotalIso (FiberSeqProj F))) ,
      cong (Iso.inv (fst (puppeTotalIso (FiberSeqProj F))))
      (sym (snd (puppeTotalIso (FiberSeqProj F))))
      ∙ Iso.leftInv (fst (puppeTotalIso (FiberSeqProj F))) _)))
 ≡⟨ cong (λ f → (Iso.fun (fst (FibsIsoFiberFiberSeq F))
          , snd (FibsIsoFiberFiberSeq F))
   ∘∙ (f ∘∙ (Iso.inv (fst (puppeTotalIso (FiberSeqProj F)))
             , cong (Iso.inv (fst (puppeTotalIso (FiberSeqProj F))))
      (sym (snd (puppeTotalIso (FiberSeqProj F))))
      ∙ Iso.leftInv (fst (puppeTotalIso (FiberSeqProj F)))
   (((pt (FiberSeqTotal F)) , (snd (FiberSeqProj F))) , refl))))
   (ProjOfFiberFiberSeq (inclOfFiberFiberSeq
                        (inclOfFiberFiberSeq (FiberSeqProj F)))
      ∙ sym (InclOfFiberFiberSeq (inclOfFiberFiberSeq (FiberSeqProj F)))) ⟩
  ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
     snd (FibsIsoFiberFiberSeq F))
    ∘∙
    (FiberSeqIncl
     (FiberFiberSeq (fiber∙ (FiberSeqProj F)) (FiberSeqTotal F)
      (inclOfFiberFiberSeq (FiberSeqProj F)))
     ∘∙
     (Iso.inv (fst (puppeTotalIso (FiberSeqProj F))) ,
      cong (Iso.inv (fst (puppeTotalIso (FiberSeqProj F))))
 ...
 ≡⟨ cong ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
           ...
           (sym (FiberIsoFiberSeqIncl
                   ...
                   (snd (puppeTotalIso (FiberSeqProj F)))
                   (FiberFiberSeq (fiber∙ (FiberSeqProj F))
                   (FiberSeqTotal F)
                   (inclOfFiberFiberSeq (FiberSeqProj F))))) ⟩
   ((Iso.fun (fst (FibsIsoFiberFiberSeq F)) ,
      snd (FibsIsoFiberFiberSeq F))
     ∘∙ FiberSeqIncl (puppeFiberFiberCase (FiberSeqProj F)))
 ≡⟨ sym (TotalIsoFiberSeqIncl (fst (FibsIsoFiberFiberSeq F))
                                (snd (FibsIsoFiberFiberSeq F))
                                (puppeFiberFiberCase (FiberSeqProj F))) ⟩
...
twiceIteratedPuppeIncl F =
   fst (FiberSeqIncl (puppe (puppe (puppe F))))
  ≡⟨ cong (λ G → fst (FiberSeqIncl G))
          (sym (altIteratedPuppeEqIteratedPuppe (puppe F))) ⟩
   fst (FiberSeqIncl (alternativeIteratedPuppe (puppe F)))
  ≡⟨ cong fst (BaseIsoFiberSeqIncl _ (snd (FibsIsoFiberFiberSeq (puppe F)))
           (alternativeIteratedPuppeFiberFiberCase (FiberSeqProj (puppe F)))) ⟩
   fst (FiberSeqIncl (alternativeIteratedPuppeFiberFiberCase
                      (FiberSeqProj (puppe F))))
  ≡⟨ cong fst (FiberIsoFiberSeqIncl
              ...
              (FiberFiberSeq _ _
              (inclOfFiberFiberSeq (inclOfFiberFiberSeq
                                    (FiberSeqProj (puppe F))))))) ⟩
   (fst (FiberSeqIncl
          (TotalIsoFiberSeq _ _ _ _
           (fst (puppeTotalIso (FiberSeqProj (puppe F))))
           ...
           (FiberFiberSeq _ _
            (inclOfFiberFiberSeq (inclOfFiberFiberSeq
                                  (FiberSeqProj (puppe F))))))))
   ∘ (Iso.inv (fst (puppeFiberIso (FiberSeqProj (puppe F)))))
  ≡⟨ cong (_∘ (Iso.inv (fst (puppeFiberIso (FiberSeqProj (puppe F))))))
     (cong fst (TotalIsoFiberSeqIncl
                (fst (puppeTotalIso (FiberSeqProj (puppe F))))
                (snd (puppeTotalIso (FiberSeqProj (puppe F))))
                (FiberFiberSeq _ _
                 (inclOfFiberFiberSeq (inclOfFiberFiberSeq
                  (FiberSeqProj (puppe F))))))) ⟩
   (Iso.fun (fst (puppeTotalIso (FiberSeqProj (puppe F)))))
   ∘ (fst (FiberSeqIncl (FiberFiberSeq _ _
                        (inclOfFiberFiberSeq (inclOfFiberFiberSeq
                                              (FiberSeqProj (puppe F)))))))
   ∘ (Iso.inv (fst (puppeFiberIso (FiberSeqProj (puppe F)))))
  ≡⟨ refl ⟩
   ...
   ∘ (fst (FiberSeqIncl (FiberFiberSeq _ _
                         (inclOfFiberFiberSeq (inclOfFiberFiberSeq
                                               (FiberSeqProj (puppe F)))))))
   ∘ (λ p → (((pt (FiberSeqFiber F)) ,
              (snd (FiberSeqProj (puppe F)))) , p) , refl)
  ≡⟨ cong (Iso.fun (fst (puppeTotalIso (FiberSeqProj (puppe F)))) ∘_)
     (cong (_∘
           (λ (p : typ (Ω (FiberSeqFiber F))) → (((pt (FiberSeqFiber F)) ,
                    (snd (FiberSeqProj (puppe F)))) , p) , refl
                 {x = (pt (FiberSeqFiber F)) , (snd (FiberSeqProj (puppe F)))}))
     (cong fst (InclOfFiberFiberSeq (inclOfFiberFiberSeq
               (inclOfFiberFiberSeq (FiberSeqProj (puppe F))))))) ⟩
   (Iso.fun (fst (puppeTotalIso (FiberSeqProj (puppe F)))))
   ∘ (fst (inclOfFiberFiberSeq (inclOfFiberFiberSeq
          (inclOfFiberFiberSeq (FiberSeqProj (puppe F))))))
   ∘ (λ p → (((pt (FiberSeqFiber F)) ,
              (snd (FiberSeqProj (puppe F)))) , p) , refl)
  ≡⟨ refl ⟩
   (Iso.fun (fst (puppeTotalIso (FiberSeqProj (puppe F)))))
   ∘ (λ p → (((pt (FiberSeqFiber F)) ,
              (snd (FiberSeqProj (puppe F)))) , p))
  ≡⟨ refl ⟩
   (λ p → (sym (snd (FiberSeqProj (puppe F))))
           ∙ cong (fst (FiberSeqProj (puppe F))) (sym p)
           ∙ snd (FiberSeqProj (puppe F)))
  ≡⟨ (λ i →
     (λ p → (sym (snd ((puppeProjEqFibIncl F) i)))
             ∙ cong (fst ((puppeProjEqFibIncl F) i)) (sym p)
...
twiceIteratedPuppeProj F =
 ...
 ≡⟨ cong fst (FiberIsoFiberSeqIncl _ _ _) ⟩
  fst (FiberSeqIncl (TotalIsoFiberSeq _ _ _ _
                      (fst (puppeTotalIso (FiberSeqProj F)))
                      (snd (puppeTotalIso (FiberSeqProj F)))
                      (FiberFiberSeq _ _
                      (inclOfFiberFiberSeq
 ...
 ≡⟨ cong (_∘ Iso.inv (fst (puppeFiberIso (FiberSeqProj F))))
    (cong fst (TotalIsoFiberSeqIncl
              (fst (puppeTotalIso (FiberSeqProj F)))
              (snd (puppeTotalIso (FiberSeqProj F)))
              (FiberFiberSeq _ _
              (inclOfFiberFiberSeq
              (inclOfFiberFiberSeq (FiberSeqProj F)))))) ⟩
  Iso.fun (fst (puppeTotalIso (FiberSeqProj F)))
  ∘ fst (FiberSeqIncl (FiberFiberSeq _ _
                       (inclOfFiberFiberSeq
                       (inclOfFiberFiberSeq (FiberSeqProj F)))))
  ∘ Iso.inv (fst (puppeFiberIso (FiberSeqProj F)))
 ≡⟨ cong (Iso.fun (fst (puppeTotalIso (FiberSeqProj F))) ∘_)
    (cong (_∘ Iso.inv (fst (puppeFiberIso (FiberSeqProj F))))
    (cong fst (InclOfFiberFiberSeq (inclOfFiberFiberSeq
               (inclOfFiberFiberSeq (FiberSeqProj F)))))) ⟩
  Iso.fun (fst (puppeTotalIso (FiberSeqProj F)))
